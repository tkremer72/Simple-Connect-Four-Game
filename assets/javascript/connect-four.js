document.addEventListener("DOMContentLoaded", () => {
  ...
  //Check the board for a win or a loss
  function checkBoard() {
    //Take the 4 values in each winning array and plug them into the squares
    for (let y = 0; i < winningArrays.length; y++) {
      ...
      if (
        squares1.classList.contains("player-one") &&
        squares2.classList.contains("player-one") &&
        squares3.classList.contains("player-one") &&
        squares4.classList.contains("player-one")
      ) {
        //If they all have the player-one class, player-one is passed as winner
        result.innerHTML = "Player 1 Wins!";
        gameReset(2500);
      } else if (
        squares1.classList.contains("player-two") &&
